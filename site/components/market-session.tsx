...
export function UpdateSchedule(){
  ...
  return <section className="update-schedule" aria-label="自動更新排程"><button type="button" className="schedule-refresh" onClick={()=>void checkForUpdate(true)} disabled={refreshing} aria-label={refreshing?'正在檢查更新':'重新整理最新發布資料'} aria-busy={refreshing} title="重新整理最新發布資料"><RefreshCw className={refreshing?'is-refreshing':''} size={21}/></button><div className="schedule-copy"><b>平日 09:00–13:30 · 每半小時更新</b><span>18:00 查核處置與收盤研究 · 台北時間 · 雲端排程可能延遲</span></div><div className="schedule-next" aria-live="polite"><b><Clock3 size={15}/>{next?'下次 '+time(next):'計算下次排程…'}</b><span>{notice}</span></div></section>;
}
export function SessionMarket({stockId}:{stockId:string}){
  ...
  })}</div><p className="session-caption">每半小時擷取，非即時串流；休市或尚無成交時沿用實際成交日期。下方估值卡片採已核對的正式收盤價，與最新成交快照分開列示。</p></section>;
}
