...
import {parseIntraday,reconcileIntradayClosing} from './kline-core.mjs';
...
try{
  for(const stock of WATCHLIST){
    ...
    const url=`https://query1.finance.yahoo.com/v8/finance/chart/${stock.id}.TW?interval=30m&range=1mo`;
    const envelope=intradayInput?await read(path.join(intradayInput,`yahoo-chart-${stock.id}-30m-1mo.json`)):await get(url);
    await save(path.join(root,'data/raw/kline/intraday',checkedAt.replaceAll(':','-'),stock.id+'.json'),envelope);
    const result=parseIntraday(envelope,stock.id,checkedAt,calendar,20);
    let intraday=reconcileIntradayClosing(result.history,daily);
    if(previous?.intraday?.at(-1)?.date>intraday.at(-1)?.date)throw new Error(stock.id+' 分時資料時間倒退，保留前次成功版本');
    const missingBars=intraday.filter(r=>r.missing).length;
    if(missingBars){check.status='pending';check.messages.push(stock.name+' 30分K 有 '+missingBars+' 根資料待確認，不繪製該棒與涉及的均線');}
    stocks.push({...stock,daily,intraday,intradayMeta:{checkedAt,fetchedAt:envelope.fetchedAt,lastCompletedAt:result.lastCompletedAt,delayMinutes:20,missingBars,source:url}});
  }
...
console.log(JSON.stringify({...check,stocks:stocks.map(s=>({id:s.id,daily:s.daily.length,intraday:s.intraday.length,missing:s.intradayMeta.missingBars}))}));
