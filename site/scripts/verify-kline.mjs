...
const epoch=s=>Date.parse(s+'+08:00')/1000,slot=m=>String(Math.floor(m/60)).padStart(2,'0')+':'+String(m%60).padStart(2,'0');
const makeEnvelope=(points,fetchedAt='2026-09-08T06:00:00Z')=>{
  const open=points.map((_,i)=>10+i/100),close=open.map(v=>v+.02),high=close.map(v=>v+.03),low=open.map(v=>v-.03),volume=points.map(()=>10);
  return {url:'https://query1.finance.yahoo.com/v8/finance/chart/2426.TW?interval=5m&range=1mo',fetchedAt,payload:{chart:{result:[{meta:{symbol:'2426.TW',exchangeTimezoneName:'Asia/Taipei',dataGranularity:'5m',regularMarketTime:epoch('2026-09-08T13:30:00')},timestamp:points.map(t=>epoch(t)),indicators:{quote:[{open,high,low,close,volume}]}}],error:null}}};
};
const regular=[];for(let minute=540;minute<810;minute+=5)regular.push('2026-09-08T'+slot(minute)+':00');regular.push('2026-09-08T13:30:00');
const envelope=makeEnvelope(regular),cal={year:2026,holidayDates:['2026-09-07']};
const early=makeEnvelope(regular,'2026-09-08T01:24:59Z');assert.equal(parseIntraday(early,'2426',early.fetchedAt,cal,5,20).history.length,0);
const boundary=makeEnvelope(regular,'2026-09-08T01:25:00Z');const boundaryResult=parseIntraday(boundary,'2426',boundary.fetchedAt,cal,5,20);assert.equal(boundaryResult.history.length,1);assert.equal(boundaryResult.history[0].date,'2026-09-08 09:00');
...
const wrong=structuredClone(envelope);wrong.payload.chart.result[0].meta.symbol='2409.TW';assert.throws(()=>parseIntraday(wrong,'2426',wrong.fetchedAt,cal,5,20));
const nulls=structuredClone(envelope);nulls.payload.chart.result[0].indicators.quote[0].close[0]=null;assert.equal(parseIntraday(nulls,'2426',nulls.fetchedAt,cal,5,20).history[0].missing,true);
const holiday=makeEnvelope(['2026-09-04T13:25:00','2026-09-08T09:00:00']);assert.equal(parseIntraday(holiday,'2426',holiday.fetchedAt,cal,5,20).history.length,2);
...
for(const stock of data.stocks){
  ...
  for(const rows of Object.values(intervals))assert.ok(rows.every(row=>!row.date.endsWith('13:30')));
}
...
const preserved=JSON.stringify({...data,stocks:data.stocks.map(s=>({...s,daily:s.daily.slice(-30),intraday:s.intraday.slice(-30),intradayByInterval:s.intradayByInterval?Object.fromEntries(Object.entries(s.intradayByInterval).map(([period,rows])=>[period,rows.slice(-30)])):undefined}))});fs.writeFileSync(path.join(testSite,'lib/kline-history.json'),preserved);
